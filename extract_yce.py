        ...
        for file in common.showProgress(files):
            ...
            with common.Stream(filepath, "rb") as f:
                ...
                for img in images:
                    ...
                    if constant != 0x1C:
                        common.logDebug("Constant is not 0x1C!", common.toHex(constant))
                    ...
                    if constant != 0x01:
                        common.logDebug("Constant 2 is not 0x01!", common.toHex(constant))
                    common.logDebug("size:", img.size, "oamnum:", img.oamnum, "oamsize:", img.oamsize)
                    common.logDebug("tilesize:", img.tilesize, "paloffset:", img.paloffset)
                    img.oams = []
                    for j in range(img.oamnum):
                        ...
                        for x in range(8):
                            unkbyte = f.readByte()
                            if unkbyte != 0x00:
                                common.logDebug("unkbyte", x, "is not 0x00!", common.toHex(unkbyte))
                        shape = f.readByte()  # NCER OBJ Shape
                        size = f.readByte()  # NCER OBJ Size
                        for x in range(2):
                            unkbyte = f.readByte()
                            if unkbyte != 0x00:
                                common.logDebug("unkbyte2", x, "is not 0x00!", common.toHex(unkbyte))
